...
namespace RegistrationForm.AdminDashvoardMdiPages.Teacher
{
    public partial class TeacherList : Form
    {
        ...
        public TeacherList()
        {
            ...
            lblCount.Text = countactive().ToString();
            lblCount.Left = (panel4.ClientSize.Width - lblCount.Width) / 2;
            if (dgvTeacher.Columns[7] is DataGridViewTextBoxColumn textBoxColumn)
            {
                textBoxColumn.MaxInputLength = 11;
            }
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            string phone = dgvTeacher.Rows[r].Cells[7].Value.ToString();
            ...
            DialogResult dr = MessageBox.Show("Are you sure you want to update this teacher ?", "Confirmation", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (dr == DialogResult.OK)
            {
                if(isDataModified == true)
                {
                    repos.UpdateTeacher(id, hiredate, depid, firstname, lastname, age, gender, phone, address, email, status);
                ...
                {
                    MessageBox.Show("No data has changed", "Abort", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
        ...
        private void dgvTeacher_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            isDataModified = true;
        }
